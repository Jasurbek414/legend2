...
const Addtablem = () => {
  ...
  async function fetchData() {
    try {
      const response = await axios.get(`api/material/category/${id}`);
  ...
  useEffect(() => {
    const productData = async () => {
      try {
        const response = await axios.get(`api/material-type`);
  ...
  async function add() {
    try {
      const response = await axios.post("api/material", sendData);
